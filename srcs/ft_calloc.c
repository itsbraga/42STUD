...
/*   Updated: 2023/05/12 14:02:03 by annabrag         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*	Description:
	Cette fonction alloue un bloc de mémoire en initialisant tous ses octets
    ...
    ⋆ Les paramètres d'appels : calloc requière deux paramètres (le nombre
								d'éléments consécutifs à allouer (nmemb) et
								la taille d'un élément (size)) alors que
								malloc attend la taille totale du bloc à
								allouer.
	
	Valeur de retour:
	Un pointeur sur la zone nouvellement allouée.
	Attention, il s'agit d'un pointeur générique (void *), il peut donc être
	nécessaire de caster le pointeur vers un autre type.
*/
